  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      packets.forEach((packet) => {
        ...
        const start = packet.edge?.geometry.attributes.position?.array as THREE.TypedArray
        if (start && start.length > 0) {
          packet.mesh.position.set(
            (start[0] as number) + ((start[3] as number) - (start[0] as number)) * packet.progress,
            (start[1] as number) + ((start[4] as number) - (start[1] as number)) * packet.progress,
            (start[2] as number) + ((start[5] as number) - (start[2] as number)) * packet.progress,
          )
        }
      })
