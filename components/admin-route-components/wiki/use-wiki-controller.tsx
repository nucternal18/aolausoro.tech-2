...
import {
  setUploadProgress,
  userSelector,
} from '@components/admin-route-components/global-redux-store/features/users/usersSlice'
import {
  isErrorWithMessage,
  isFetchBaseQueryError,
} from '@components/admin-route-components/global-redux-store/helper'

import { ToastAction } from '@components/ui/toast'
import { createWiki, getWiki } from '@components/admin-route-components/actions/wiki'
import { uploadUserImage } from '@components/admin-route-components/actions/upload'
...
export default function useWikiController() {
  ...
  const uploadImage: SubmitHandler<FormValues> = useCallback(async (values) => {
    ...
    try {
      const { data: uploadPDFResponse, error, progress } = await uploadUserImage(url, formData)
      ...
      if (isFetchBaseQueryError(error)) {
        const errMsg = 'error' in error ? error.error : JSON.stringify(error.message)
        toast({
          title: 'Error!',
          description: (errMsg as string) || 'Unable to upload PDF. Please try again.',
