'use server'
import type { JobsProps, PartialJobProps, StatsProps } from '@src/entities/models/Job'
...
export async function getStats(): Promise<StatsProps> {
  return await withServerActionInstrumentation('getStats', { recordResponse: true }, async () => {
...
export async function getJobs(queryItems: QueryItemsProps): Promise<JobsProps> {
  return await withServerActionInstrumentation('getJobs', { recordResponse: true }, async () => {
    const { userId } = await auth()
    try {
...
export async function getJobById(id: string): Promise<PartialJobProps> {
  return await withServerActionInstrumentation('getJobById', { recordResponse: true }, async () => {
    const { userId } = await auth()
    try {
...
export async function createJob(input: FormData): Promise<{ success: boolean; message: string }> {
  return await withServerActionInstrumentation('createJob', { recordResponse: true }, async () => {
    const { userId } = await auth()
    try {
...
export async function updateJob(input: FormData): Promise<{ success: boolean; message: string }> {
  return await withServerActionInstrumentation('updateJob', { recordResponse: true }, async () => {
    const { userId } = await auth()
    try {
      const formData = Object.fromEntries(input.entries())
...
export async function deleteJob(id: string): Promise<{ success: boolean; message: string }> {
  return await withServerActionInstrumentation('deleteJob', { recordResponse: true }, async () => {
    const { userId } = await auth()
    try {
