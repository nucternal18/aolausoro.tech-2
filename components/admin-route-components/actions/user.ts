...
import { updateUserController, getUserController } from '@src/interface-adapters/controllers/user'

export async function getUser() {
  return await withServerActionInstrumentation('getUser', { recordResponse: false }, async () => {
...
export async function updateUser(requestBody: FormData) {
  return await withServerActionInstrumentation('createTodo', { recordResponse: true }, async () => {
    const { userId } = await auth()
    try {
