...
export async function getMessageById(id: string): Promise<PartialMessageProps | undefined> {
  return await withServerActionInstrumentation(
    'getMessageById',
    { recordResponse: false },
    async () => {
  ...
  return await withServerActionInstrumentation(
    ...
    async () => {
      ...
      try {
        ...
        const response = await createMessageController(formData, userId as string)
        if (response.success) {
          revalidatePath('/')
        }
        return response
...
export async function sendMail(input: FormData): Promise<{ success: boolean; message: string }> {
  return await withServerActionInstrumentation('sendMail', { recordResponse: true }, async () => {
    try {
      const formData = Object.fromEntries(input.entries())
