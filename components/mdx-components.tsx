...
export const mdxComponents = {
  h1: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <h1
      className={cn('text-primary mt-2 scroll-m-20 text-4xl font-bold tracking-tight', className)}
      {...props}
    />
  ),
  h2: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <h2
      className={cn(
        'text-primary mt-10 scroll-m-20 border-b pb-1 text-3xl font-semibold tracking-tight first:mt-0',
        className,
      )}
      {...props}
    />
  ...
  h4: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <h4
      className={cn(
        'text-primary mt-8 scroll-m-20 text-xl font-semibold tracking-tight',
        className,
      )}
      {...props}
    />
  ...
  p: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <p className={cn('text-primary leading-7 [&:not(:first-child)]:mt-6', className)} {...props} />
  ),
  ul: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <ul className={cn('text-primary my-6 ml-6 list-disc', className)} {...props} />
  ),
  ol: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <ol className={cn('text-primary my-6 ml-6 list-decimal', className)} {...props} />
  ),
  ...
  code: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <code
      className={cn('relative px-[0.6rem] py-[0.2rem] font-mono text-sm', className)}
      {...props}
    />
  ),
  Image,
