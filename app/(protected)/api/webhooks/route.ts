...
export async function POST(req: Request) {
  try {
    ...
    if (!WEBHOOK_SECRET) {
      throw new Error('Please add the Clerk Webhook Secret to your environment variables.')
    }
    ...
    const userData =
      'user' in evt.data && evt.data.user ? (evt.data.user as UserJSON) : ({} as UserJSON)
    ...
    if (eventType === 'user.updated') {
      ...
      await prisma.user.update({
        ...
        data: {
          ...
          image: userData.has_image ? userData.image_url : foundUser.image,
          name: `${userData.first_name} ${userData.last_name}`,
        },
      })
